...
void clean_database_fixture::resize_shared_mem( uint64_t size )
{
   ...
   db.open( data_dir->path(), INITIAL_TEST_SUPPLY, size, chainbase::database::read_write );
...
void database_fixture::open_database()
{
   if( !data_dir ) {
      ...
      db.open( data_dir->path(), INITIAL_TEST_SUPPLY, 1024 * 1024 * 8, chainbase::database::read_write ); // 8 MB file for testing
   }
}
