...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      // Keep SBD Print Rate 100% after HF 19_9999
      BOOST_TEST_MESSAGE( "Checking printing SBD has slowed pre HF 19__9999" );
      if( db.has_hardfork( STEEMIT_HARDFORK_0_19__9999 ) ) {
         BOOST_REQUIRE( db.get_dynamic_global_properties().sbd_print_rate == STEEMIT_100_PERCENT );
      } else {
         BOOST_REQUIRE( db.get_dynamic_global_properties().sbd_print_rate < STEEMIT_100_PERCENT );
      }
      ...
      BOOST_TEST_MESSAGE( "Letting percent market cap fall to 2% to verify printing of SBD turns back on pre HF 19_9999" );
