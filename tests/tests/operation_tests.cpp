...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( DPN_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = DPN_INIT_MINER_NAME;
      ...
      BOOST_REQUIRE( acct.dbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( DPN_INIT_MINER_NAME ).balance.amount + 1, DPN_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( DPN_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( DPN_INIT_MINER_NAME, DPN_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = DPN_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / DPN_BLOCK_INTERVAL );
      ...
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.signatures.clear();
      sign( tx, bob_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( DPN_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / DPN_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = dpn::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / DPN_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( DPN_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DPN_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( DPN_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / DPN_BLOCK_INTERVAL );
      ...
      generate_blocks( DPN_CASHOUT_WINDOW_SECONDS / DPN_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * DPN_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - DPN_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - DPN_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db->head_block_time() + DPN_MIN_VOTE_INTERVAL_SEC );

         util::manabar old_manabar = db->get_account( "alice" ).voting_manabar;
         util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), DPN_VOTING_MANA_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - DPN_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db->head_block_time() + fc::seconds( ( DPN_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - DPN_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         util::manabar_params downvote_params( util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4, DPN_VOTING_MANA_REGENERATION_SECONDS );
         old_downvote_manabar.regenerate_mana( downvote_params, db->head_block_time() );
         int64_t sam_weight = old_downvote_manabar.current_mana - db->get_account( "sam" ).downvote_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD;
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + DPN_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (DPN_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / DPN_VOTING_MANA_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / DPN_100_PERCENT ).to_uint64() - DPN_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + DPN_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( DPN_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / DPN_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( DPN_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / DPN_100_PERCENT;
         ...
         new_rshares = old_downvote_manabar.current_mana - db->get_account( "alice" ).downvote_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD;
...

         {
            ...
            auto max_vote_denom = db->get_dynamic_global_properties().vote_power_reserve_rate * DPN_VOTING_MANA_REGENERATION_SECONDS;
...

         {
            const auto& bob_comment = db->get_comment( "bob", string( "foo" ) );
            const auto& alice = db->get_account( "alice" );
            BOOST_REQUIRE( bob_comment.net_rshares == old_net_rshares - alice_weight + DPN_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( bob_comment.abs_rshares == old_abs_rshares + alice_weight - DPN_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( alice_bob_vote->rshares == -1 * ( alice_weight - DPN_VOTE_DUST_THRESHOLD ) );
            BOOST_REQUIRE( alice_bob_vote->last_update == db->head_block_time() );
            BOOST_REQUIRE( alice_bob_vote->vote_percent == op.weight );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - DPN_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + DPN_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( DPN_UPVOTE_LOCKOUT_SECONDS - DPN_BLOCK_INTERVAL ) ) / DPN_UPVOTE_LOCKOUT_SECONDS;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( DPN_UPVOTE_LOCKOUT_SECONDS - DPN_BLOCK_INTERVAL - DPN_BLOCK_INTERVAL ) ) / DPN_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + DPN_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );

      BOOST_REQUIRE( new_alice.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( new_bob.balance.amount.value == ASSET( "10.000 TESTS" ).amount.value );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test failure transfering DPN to dpn.dao" );
      op.from = "bob";
      op.to = DPN_TREASURY_ACCOUNT;
      op.amount = ASSET( "1.000 TESTS" );
      tx.signatures.clear();
      tx.operations.clear();
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx ), fc::exception );

      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DPN_TREASURY_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test transfering DBD to dpn.dao" );
      auto treasury_dbd_balance = db->get_account( DPN_TREASURY_ACCOUNT ).dbd_balance;
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).dbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( DPN_TREASURY_ACCOUNT ).dbd_balance == treasury_dbd_balance + ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_dpn.amount, DPN_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_dpn.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, DPN_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DPN_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( DPN_VESTING_WITHDRAW_INTERVALS * 2 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + DPN_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( DPN_VESTING_WITHDRAW_INTERVALS * 3 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + DPN_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_dpn;
            gpo.total_vesting_fund_dpn = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_dpn_asset::from_asset( asset(DPN_MIN_ACCOUNT_CREATION_FEE + 10, DPN_SYMBOL) );
      op.props.maximum_block_size = DPN_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == DPN_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == DPN_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == DPN_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = DPN_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == DPN_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = DPN_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( DPN_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( DPN_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 TBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 DPN : 1 DBD
      ...
      BOOST_REQUIRE( alice_witness.dbd_exchange_rate == op.exchange_rate );
      BOOST_REQUIRE( alice_witness.last_dbd_exchange_update == db->head_block_time() );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- Test failure publishing with DBD base symbol" );
      ...
      // BOOST_REQUIRE( std::abs( alice_witness.dbd_exchange_rate.to_real() - op.exchange_rate.to_real() ) < 0.0000005 );
      BOOST_REQUIRE( alice_witness.last_dbd_exchange_update == db->head_block_time() );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( new_bob.dbd_balance.amount.value == ASSET( "7.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( new_alice.dbd_balance.amount.value == ASSET( "2.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- Test success converting DBD to TESTS" );
      op.owner = "bob";
      ...
      BOOST_REQUIRE( new_bob.balance.amount.value == ASSET( "3.000 TESTS" ).amount.value );
      BOOST_REQUIRE( new_bob.dbd_balance.amount.value == ASSET( "4.000 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + DPN_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      DPN_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "1000.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DPN_SYMBOL ) );
      ...
      // Alice has order for 15 DBD at a price of 2:3
      // Fill 5 DPN for 7.5 DBD
      ...
      BOOST_REQUIRE( limit_order_idx.find( boost::make_tuple( "bob", op.orderid ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "7.500 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "5.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "992.500 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "15.000 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "10.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "977.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "22.500 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "33.500 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "25.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "965.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "45.500 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "35.909 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "954.500 TBD" ).amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        DPN_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        DPN_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        DPN_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( DPN_INIT_MINER_NAME, "bob", asset( 1000, DPN_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + DPN_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + DPN_OWNER_UPDATE_LIMIT + fc::seconds( DPN_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + DPN_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - DPN_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( DPN_OWNER_AUTH_RECOVERY_PERIOD - DPN_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      DPN_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      DPN_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      DPN_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + DPN_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( DPN_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      DPN_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      DPN_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.from = "alice";
      op.to = "bob";
      op.dbd_amount = ASSET( "1.000 TBD" );
      op.dpn_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
      ...
      BOOST_TEST_MESSAGE( "--- failure when dbd symbol != DBD" );
      op.dbd_amount.symbol = DPN_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when dpn symbol != DPN" );
      op.dbd_amount.symbol = DBD_SYMBOL;
      op.dpn_amount.symbol = DBD_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != DBD and fee symbol != DPN" );
      op.dpn_amount.symbol = DPN_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when dbd == 0 and dpn == 0" );
      op.fee.symbol = DPN_SYMBOL;
      op.dbd_amount.amount = 0;
      op.dpn_amount.amount = 0;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when dbd < 0" );
      op.dbd_amount.amount = -100;
      op.dpn_amount.amount = 1000;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when dpn < 0" );
      op.dbd_amount.amount = 1000;
      op.dpn_amount.amount = -100;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when from cannot cover dbd amount" );
      signed_transaction tx;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DPN_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      auto alice_dpn_balance = alice.balance - op.dpn_amount - op.fee;
      auto alice_dbd_balance = alice.dbd_balance - op.dbd_amount;
      auto bob_dpn_balance = bob.balance;
      auto bob_dbd_balance = bob.dbd_balance;
      auto sam_dpn_balance = sam.balance;
      auto sam_dbd_balance = sam.dbd_balance;
      ...
      BOOST_REQUIRE( escrow.dbd_balance == op.dbd_amount );
      BOOST_REQUIRE( escrow.dpn_balance == op.dpn_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_dpn_balance );
      BOOST_REQUIRE( alice.dbd_balance == alice_dbd_balance );
      BOOST_REQUIRE( bob.balance == bob_dpn_balance );
      BOOST_REQUIRE( bob.dbd_balance == bob_dbd_balance );
      BOOST_REQUIRE( sam.balance == sam_dpn_balance );
      BOOST_REQUIRE( sam.dbd_balance == sam_dbd_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.dpn_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.dbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( escrow.dpn_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DPN_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      DPN_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + DPN_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.dpn_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + DPN_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * DPN_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.ratification_deadline == et_op.ratification_deadline );
      BOOST_REQUIRE( escrow.escrow_expiration == et_op.escrow_expiration );
      BOOST_REQUIRE( escrow.dbd_balance == et_op.dbd_amount );
      BOOST_REQUIRE( escrow.dpn_balance == et_op.dpn_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when dpn < 0" );
      op.dpn_amount.amount = -1;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when dbd < 0" );
      op.dpn_amount.amount = 0;
      op.dbd_amount.amount = -1;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when dpn == 0 and dbd == 0" );
      op.dbd_amount.amount = 0;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when dbd is not dbd symbol" );
      op.dbd_amount = ASSET( "1.000 TESTS" );
      DPN_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when dpn is not dpn symbol" );
      op.dbd_amount.symbol = DBD_SYMBOL;
      op.dpn_amount = ASSET( "1.000 TBD" );
      DPN_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.dpn_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).dpn_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).dpn_balance == ASSET( "0.800 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "0.100 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- failure when releasing more dbd than available" );
      op.dpn_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less dpn than available" );
      op.dpn_amount = ASSET( "0.000 TESTS" );
      op.dbd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpn_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpn_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpn_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.dpn_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      DPN_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpn_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).dpn_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.dpn_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is DBD" );
      op.amount = ASSET( "1.000 TBD" );
      op.validate();


      BOOST_TEST_MESSAGE( "success when amount is DPN" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "10.000 TBD" ) );
      ...
      BOOST_TEST_MESSAGE( "--- Failure when transferring to dpn.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring DPN to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring DBD to self" );
      op.amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "9.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "1.000 TBD" ) );
      validate_database();


      BOOST_TEST_MESSAGE( "--- success transferring DPN to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring DBD to other" );
      op.amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "8.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).savings_dbd_balance == ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Failure withdrawing TESTS to dpn.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- Success withdrawing TBD to dpn.dao" );
      ...
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "9.000 TBD" ) );
      validate_database();

      BOOST_TEST_MESSAGE( "--- success withdrawing DPN to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id++;
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + DPN_SAVINGS_WITHDRAW_TIME );
      validate_database();


      BOOST_TEST_MESSAGE( "--- success withdrawing DBD to self" );
      op.amount = ASSET( "1.000 TBD" );
      op.request_id++;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "8.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == op.request_id + 1 );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).from == op.from );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).to == op.to );
      BOOST_REQUIRE( to_string( db->get_savings_withdraw( "alice", op.request_id ).memo ) == op.memo );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing DPN to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing DBD to other" );
      op.amount = ASSET( "1.000 TBD" );
      op.request_id = 4;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "7.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == op.request_id + 1 );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).from == op.from );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).to == op.to );
      BOOST_REQUIRE( to_string( db->get_savings_withdraw( "alice", op.request_id ).memo ) == op.memo );
      ...
      generate_blocks( db->head_block_time() + DPN_SAVINGS_WITHDRAW_TIME - fc::seconds( DPN_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).dbd_balance == ASSET( "1.000 TBD" ) );
      ...
      for( int i = 0; i < DPN_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = DPN_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == DPN_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + DPN_OWNER_AUTH_RECOVERY_PERIOD );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_dpn = ASSET( "0.000 TESTS" );
      op.reward_dbd = ASSET( "0.000 TBD" );
      ...
      op.reward_dpn.amount = 1000;
      op.validate();

      op.reward_dpn.amount = 0;
      op.reward_dbd.amount = 1000;
      op.validate();

      op.reward_dbd.amount = 0;
      ...
      BOOST_TEST_MESSAGE( "Testing wrong DPN symbol" );
      op.reward_dpn = ASSET( "1.000 TBD" );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong DBD symbol" );
      op.reward_dpn = ASSET( "1.000 TESTS" );
      op.reward_dbd = ASSET( "1.000 TESTS" );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong VESTS symbol" );
      op.reward_dbd = ASSET( "1.000 TBD" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Attempting to claim more DPN than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_dpn = ASSET( "20.000 TESTS" );
      op.reward_dbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_dpn + op.reward_dpn );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dpn_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == alice_dbd + op.reward_dbd );
      ...
      op.reward_dpn = ASSET( "10.000 TESTS" );
      op.reward_dbd = ASSET( "10.000 TBD" );
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      vest( DPN_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "bob" ) ), DPN_VOTING_MANA_REGENERATION_SECONDS );
      old_manabar.regenerate_mana( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - DPN_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      vest( DPN_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DPN_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, sam_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( gpo.delegation_return_period == DPN_DELEGATION_RETURN_PERIOD_HF20 );
      ...
      generate_blocks( exp_obj->expiration + DPN_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DPN_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DPN_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), DPN_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DPN_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DPN_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DPN_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", DPN_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get_account( DPN_TREASURY_ACCOUNT ), [=]( account_object& a )
         {
            a.dbd_balance.amount.value = 0;
         });
      });
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DPN_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( DPN_INIT_MINER_NAME + fc::to_string( i ) ), DPN_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), DPN_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * DPN_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * DPN_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( DPN_TREASURY_ACCOUNT ), 10 * DPN_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * DPN_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - DPN_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_dpn;
            gpo.total_reward_fund_dpn = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_dpn;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_dpn.amount + db->get_account( "bob" ).reward_dbd_balance.amount + db->get_account( "sam" ).reward_vesting_dpn.amount + db->get_account( "sam" ).reward_dbd_balance.amount + db->get_account( DPN_TREASURY_ACCOUNT ).dbd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_dbd_balance.amount + db->get_account( "alice" ).reward_vesting_dpn.amount + db->get_account( DPN_TREASURY_ACCOUNT ).dbd_balance.amount ) == db->get_account( "bob" ).reward_vesting_dpn.amount + db->get_account( "bob" ).reward_dbd_balance.amount + 1 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_dbd_balance.amount + db->get_account( "alice" ).reward_vesting_dpn.amount + db->get_account( DPN_TREASURY_ACCOUNT ).dbd_balance.amount ) == ( db->get_account( "sam" ).reward_vesting_dpn.amount + db->get_account( "sam" ).reward_dbd_balance.amount ) / 2 + 1 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_dpn.amount == db->get_account( "bob" ).reward_dbd_balance.amount + 1 );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_vesting_dpn.amount == db->get_account( "sam" ).reward_dbd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      DPN_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below DPN_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( DPN_MIN_BLOCK_SIZE_LIMIT - 1 );
      DPN_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting dbd_interest_rate with negative number" );
      prop_op.props.erase( "maximum_block_size" );
      prop_op.props[ "dbd_interest_rate" ] = fc::raw::pack_to_vector( -700 );
      DPN_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting dbd_interest_rate to DPN_100_PERCENT + 1" );
      prop_op.props[ "dbd_interest_rate" ].clear();
      prop_op.props[ "dbd_interest_rate" ] = fc::raw::pack_to_vector( DPN_100_PERCENT + 1 );
      DPN_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new dbd_exchange_rate with DBD / DPN" );
      prop_op.props.erase( "dbd_interest_rate" );
      prop_op.props[ "dbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET( "1.000 TESTS" ), ASSET( "10.000 TBD" ) ) );
      DPN_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new url with length of zero" );
      prop_op.props.erase( "dbd_exchange_rate" );
      ...
      uint64_t unit = uint64_t(1) << DPN_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / DPN_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / DPN_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/DPN_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, DPN_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == DPN_MIN_BLOCK_SIZE_LIMIT + 1 );

      // Setting dbd_interest_rate
      prop_op.props.erase( "maximum_block_size" );
      prop_op.props[ "dbd_interest_rate" ] = fc::raw::pack_to_vector( 700 );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.dbd_interest_rate == 700 );
      ...
      // Setting new dbd_exchange_rate
      prop_op.props.erase( "new_signing_key" );
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "dbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET(" 1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.dbd_exchange_rate == price( ASSET( "1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
      BOOST_REQUIRE( alice_witness.last_dbd_exchange_update == db->head_block_time() );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( DPN_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == DPN_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << DPN_RD_DECAY_DENOM_SHIFT ) / DPN_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( DPN_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( DPN_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - DPN_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - DPN_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> DPN_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( DPN_TEMP_ACCOUNT ), [&]( account_object& a )
         {
...
BOOST_AUTO_TEST_CASE( account_auth_tests )
{
   try
   {
      ...
      vest( DPN_INIT_MINER_NAME, "alice" , ASSET( "10.000 TESTS" ) );
      vest( DPN_INIT_MINER_NAME, "bob" , ASSET( "10.000 TESTS" ) );
      vest( DPN_INIT_MINER_NAME, "charlie" , ASSET( "10.000 TESTS" ) );
      generate_block();
