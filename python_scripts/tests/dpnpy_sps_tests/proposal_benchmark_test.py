...
#example A:
#"/home/a/dpnd" Path to dpnd exe
# "/home/a/data"  Path to blockchain directory
# "./dpn_utils/resources/config.ini.in" Path to config.ini.in - usually ./dpn_utils/resources/config.ini.in
#  1 - number of proposals for every account
#  200 - number of accounts
#  200000.000- number of DPN's for every account
#  30000.000- number of DBD's for every account
...
#  ./proposal_benchmark_test.py "/home/a/dpnd" "/home/a/data" "./dpn_utils/resources/config.ini.in" initminer 1 200 200000.000 30000.000 100000.000
...
#  ./proposal_benchmark_test.py "/home/a/dpnd" "/home/a/data" "./dpn_utils/resources/config.ini.in" initminer 2 300 200000.000 30000.000 100000.000
...
def create_accounts(node, creator, accounts):
    for account in accounts:
        ...
        node.commit.create_account(account['name'], 
            ...
            creator=creator,
            asset='TESTS')
    dpn_utils.dpn_tools.wait_for_blocks_produced(delayed_blocks, node.url)
...
def transfer_assets_to_accounts(node, from_account, accounts, dpns, dbds):
    for acnt in accounts:
        logger.info("Transfer from {} to {} amount {} {}".format(from_account, acnt['name'], dpns, "TESTS"))
        node.commit.transfer(acnt['name'], dpns, "TESTS", memo = "initial transfer", account = from_account)
    dpn_utils.dpn_tools.wait_for_blocks_produced(delayed_blocks, node.url)
    for acnt in accounts:
        logger.info("Transfer from {} to {} amount {} {}".format(from_account, acnt['name'], dbds, "TBD"))
        node.commit.transfer(acnt['name'], dbds, "TBD", memo = "initial transfer", account = from_account)
    dpn_utils.dpn_tools.wait_for_blocks_produced(delayed_blocks, node.url)

def create_permlink( node, account ):
   return "dpnpy-proposal-title-{}".format( account )

def create_posts(node, accounts):
    logger.info("Creating posts...")
    i = 0
    for acnt in accounts:
        node.commit.post("Dpnpy proposal title [{}]".format(acnt['name']),
            "Dpnpy proposal body [{}]".format(acnt['name']),
            acnt['name'], 
            permlink = create_permlink( node, acnt['name'] ), 
            tags = "proposals")
        i += 1
        if ( i % data_size ) == 0:
            dpn_utils.dpn_tools.wait_for_blocks_produced(delayed_blocks_ex, node.url)
...
def generate_dates( node ):
    ...
    #because of DPN_PROPOSAL_MAINTENANCE_CLEANUP
...
def vote_proposals(node, ids, accounts ):
    ...
    proposal_limit = DPN_PROPOSAL_MAX_IDS_NUMBER
...
def get_block_time( node ):
    global_properties = dpn_utils.dpn_tools.get_dynamic_global_properties( node.url )
...
if __name__ == '__main__':
    ...
    parser.add_argument("path", type=str, help = "Path to dpnd")
    parser.add_argument("dir", type=str, help = "Path to blockchain directory")
    parser.add_argument("ini", type=str, help = "Path to config.ini.in - usually ./dpn_utils/resources/config.ini.in")
    ...
    parser.add_argument("dpns", type=str, default="200000.000", help = "DPN's")
    parser.add_argument("dbds", type=str, default="30000.000", help = "DBD's")
    parser.add_argument("vests", type=str, default="100000.000", help = "VEST's")

    args = parser.parse_args()

    node = dpn_utils.dpn_runner.DpnNode( args.path, args.dir, args.ini )
    ...
    try:
        if node.is_running():
            node_client = Dpn(nodes = [node_url], no_broadcast = False, keys = keys)

            create_accounts(node_client, args.creator, accounts)
            transfer_to_vesting(node_client, args.creator, accounts, args.vests )
            transfer_assets_to_accounts(node_client, args.creator, accounts, args.dpns, args.dbds )
