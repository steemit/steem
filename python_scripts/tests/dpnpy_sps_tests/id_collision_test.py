...
class ProposalsCreatorThread(threading.Thread):
    def __init__(self, node_url, proposals, private_key, delay):
        ...
        self.node_client = Dpn(nodes = [self.node_url], keys = [self.private_key])
...
def list_proposals_by_node(creator, private_key, nodes, subjects):
    for idx in range(0, len(nodes)):
        ...
        s = Dpn(nodes = [node], keys = [private_key])
...
if __name__ == "__main__":
    ...
    parser.add_argument("nodes_url", type=str, nargs="+", help="Url of working dpn node")
    ...
    node_client = Dpn(nodes = args.nodes_url, keys = [args.wif])
    logger.info("New post ==> ({},{},{},{},{})".format(
        "Dpnpy proposal title [{}]".format(args.creator),
        "Dpnpy proposal body [{}]".format(args.creator),
        args.creator, 
        get_permlink(args.creator), 
        "proposals"
    ))

    node_client.commit.post("Dpnpy proposal title [{}]".format(args.creator),
        "Dpnpy proposal body [{}]".format(args.creator),
        args.creator, 
    ...
    dpn_utils.dpn_tools.wait_for_blocks_produced(5, args.nodes_url[0])
    ...
    for idx in range(0, len(node_subjects)):
        ...
        s = Dpn(nodes = [node], keys = [args.wif])
