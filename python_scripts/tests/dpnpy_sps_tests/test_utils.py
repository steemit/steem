...
MODULE_NAME = "SPS-Tester-via-dpnpy.SPS-Tester-Utils"
...
def transfer_assets_to_treasury(node, from_account, treasury_account, amount, asset):
    ...
    dpn_utils.dpn_tools.wait_for_blocks_produced(2, node.url)
...
def create_posts(node, accounts):
    ...
    for acnt in accounts:
        logger.info("New post ==> ({},{},{},{},{})".format(
            "Dpnpy proposal title [{}]".format(acnt['name']),
...
def print_balance(node, accounts):
    ...
    for acnt in accounts:
        ...
        dbd = ret.get('dbd_balance', 'Error')
        balances_str.append("{}:{}".format(acnt['name'], dbd))
        balances.append(dbd)
