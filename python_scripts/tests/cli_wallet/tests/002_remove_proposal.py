...
if __name__ == "__main__":
    try:
        ...
        wallet.create_account("initminer", "rmtest", "", "true")
        json = last_message_as_json( wallet.list_proposals("initminer", "creator", "asc", 20, 1))
        proposals_before = len(json["result"])

        log.info("proposals_before {0}".format(proposals_before))

        wallet.create_proposal("initminer", "rmtest", "2029-06-02T00:00:00", "2029-08-01T00:00:00", "1.000 TBD", "this is subject", "http://url.html", "true")
        ...
        wallet.remove_proposal("initminer", [new_proposal_id], "true", "true")
        json = last_message_as_json(wallet.list_proposals("initminer", "creator", "asc", 20, 1))
