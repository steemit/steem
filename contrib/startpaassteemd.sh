...
if [[ "$USE_RAMDISK" ]]; then
  ...
  do
    ...
    if [[ "$IS_BROADCAST_NODE" ]]; then
      echo steemd: beginning download and decompress of s3://$S3_BUCKET/broadcast-$VERSION-latest.tar.lz4
      ...
      echo steemd: beginning download and decompress of s3://$S3_BUCKET/ahnode-$VERSION-latest.tar.lz4
