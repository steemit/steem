...
#include <mira/detail/promotes_arg.hpp>
#include <utility>
...
using boost::multi_index::detail::ordered_index_find;
...
inline Node* ordered_index_find(
  ...
  const CompatibleCompare& comp,boost::mpl::true_)
{
  return ordered_index_find(top,y,key,x,comp,boost::mpl::false_());
}
...
inline Node* ordered_index_find(
  Node* top,Node* y,const KeyFromValue& key,const CompatibleKey& x,
  const CompatibleCompare& comp,boost::mpl::false_)
{
  Node* y0=y;
...
using boost::multi_index::detail::ordered_index_lower_bound;
...
inline Node* ordered_index_lower_bound(
  ...
  const CompatibleCompare& comp,boost::mpl::true_)
{
  return ordered_index_lower_bound(top,y,key,x,comp,boost::mpl::false_());
}
...
using boost::multi_index::detail::ordered_index_upper_bound;
/*
template<
  typename Node,typename KeyFromValue,
  typename CompatibleKey,typename CompatibleCompare
...
inline Node* ordered_index_upper_bound(
  ...
  const CompatibleCompare& comp,boost::mpl::true_)
{
  return ordered_index_upper_bound(top,y,key,x,comp,boost::mpl::false_());
}
...
using boost::multi_index::detail::ordered_index_equal_range;
...
inline std::pair<Node*,Node*> ordered_index_equal_range(
  ...
  const CompatibleCompare& comp,boost::mpl::true_)
{
  return ordered_index_equal_range(top,y,key,x,comp,boost::mpl::false_());
}
    ...
    if(comp(key(top->value()),x)){
      ...
      return std::pair<Node*,Node*>(
        ordered_index_lower_bound(
          Node::from_impl(top->left()),top,key,x,comp,boost::mpl::false_()),
        ordered_index_upper_bound(
          Node::from_impl(top->right()),y,key,x,comp,boost::mpl::false_()));
    }
  }
