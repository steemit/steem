   ...
   const rc_curve_gen_params& curve_gen_params )
{
   ...
   uint64_t A_num = uint64_t( curve_gen_params.a_point_num ) * ( uint64_t(curve_gen_params.u_point_denom) - uint64_t(curve_gen_params.u_point_num) );
   uint64_t A_denom = uint32_t( curve_gen_params.a_point_denom ) * uint32_t( curve_gen_params.u_point_num ) * uint64_t(resource_dynamics_params.decay_params.decay_per_time_unit);
   ...
   int8_t shift = shift1+shift2;
   FC_ASSERT( shift >= STEEM_RD_DECAY_DENOM_SHIFT );
   u128_A <<= shift;
   u128_A /= A_denom;
   ...
   price_curve_params.shift = uint8_t( shift - STEEM_RD_DECAY_DENOM_SHIFT );
