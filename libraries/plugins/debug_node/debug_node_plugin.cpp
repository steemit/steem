        ...
        namespace debug_node {
            ...
            uint32_t debug_node_plugin::debug_generate_blocks(const std::string &debug_key, uint32_t count, uint32_t skip, uint32_t miss_blocks) {
                ...
                for (uint32_t i = 0; i < count; i++) {
                    std::string scheduled_witness_name = db.get_scheduled_witness(
                            miss_blocks + 1);
                    fc::time_point_sec scheduled_time = db.get_slot_time(
                            miss_blocks + 1);
            ...
            uint32_t debug_node_plugin::debug_generate_blocks_until(const std::string &debug_key, const fc::time_point_sec &head_block_time, bool generate_sparsely, uint32_t skip) {
                ...
                FC_ASSERT(head_block_time.sec_since_epoch() -
                          db.head_block_time().sec_since_epoch() <
                          STEEMIT_BLOCK_INTERVAL, "", ("desired_time", head_block_time)("db.head_block_time()", db.head_block_time()));
                return new_blocks;
            }
            ...
            void debug_node_plugin::on_changed_objects(const std::vector<graphene::db::object_id_type> &ids) {
                ...
                if (_json_object_stream && (ids.size() > 0)) {
                    ...
                    for (const graphene::db::object_id_type &oid : ids) {
                        ...
                        if (obj == nullptr) {
                            (*_json_object_stream) << "{\"id\":"
                                                   << fc::json::to_string(oid)
                                                   << "}\n";
                        } else {
                            (*_json_object_stream)
                                    << fc::json::to_string(obj->to_variant())
                                    << '\n';
                        }
                    }
                }
            ...
            void debug_node_plugin::on_applied_block(const chain::signed_block &b) {
                ...
                if (_json_object_stream) {
                    (*_json_object_stream) << "{\"bn\":"
                                           << fc::to_string(b.block_num())
                                           << "}\n";
                }
            }
