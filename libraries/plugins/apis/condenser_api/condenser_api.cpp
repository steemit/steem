...
condenser_api::condenser_api( steemit::chain::condenser& db, bool disable_get_block )
   : _api( new steemit::app::condenser_api( db, disable_get_block ) )
{
...
DEFINE_API( condenser_api, get_miner_queue )
{
...
DEFINE_API( condenser_api, get_liquidity_queue )
{
   FC_ASSERT( args.size() == 1 || args.size() == 2, "Expected 1-2 arguments, was ${n}", ("n", args.size()) );
