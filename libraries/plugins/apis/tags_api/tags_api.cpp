...
DEFINE_API( tags_api_impl, get_discussions_by_feed )
{
   ...
   {
      auto start_c = c_idx.find( boost::make_tuple( _db.get_comment( start_author, start_permlink ).id, account.name, 1/*activated*/ ) );
   ...
   while( result.discussions.size() < args.limit && feed_itr->activated ==1 && feed_itr != f_idx.end() )
   {
