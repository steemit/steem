...
namespace detail {
   ...
   void witness_plugin_impl::schedule_production_loop() {
      ...
      // If we would wait less than 50ms, wait for at least another 200ms.
      fc::time_point now = fc::time_point::now();
      int64_t time_to_next_second = 1000000 - (now.time_since_epoch().count() % 1000000);
      if(time_to_next_second < 50000)      // we must sleep for at least 50ms
         time_to_next_second += 200000;
      else if(time_to_next_second > 200000)  // we must sleep for at least 200ms
         time_to_next_second = 200000;
   ...
   //         ilog("Not producing block because slot has not yet arrived");
         ...
         case block_production_condition::lag:
            elog("Not producing block because node didn't wake up within 750ms of the slot time.");
            break;
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      if( llabs((scheduled_time - now).count()) > fc::milliseconds( 750 ).count() )
      {
