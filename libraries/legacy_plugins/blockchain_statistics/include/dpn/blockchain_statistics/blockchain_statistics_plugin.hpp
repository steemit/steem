...
// time.
//
#ifndef DPN_BLOCKCHAIN_STATISTICS_SPACE_ID
#define DPN_BLOCKCHAIN_STATISTICS_SPACE_ID 9
#endif

#ifndef DPN_BLOCKCHAIN_STATISTICS_PLUGIN_NAME
#define DPN_BLOCKCHAIN_STATISTICS_PLUGIN_NAME "chain_stats"
#endif

namespace dpn { namespace blockchain_statistics {
...
{
   bucket_object_type = ( DPN_BLOCKCHAIN_STATISTICS_SPACE_ID << 8 )
};
...
class blockchain_statistics_plugin : public dpn::app::plugin
{
   public:
      blockchain_statistics_plugin( application* app );
      virtual ~blockchain_statistics_plugin();

      virtual std::string plugin_name()const override { return DPN_BLOCKCHAIN_STATISTICS_PLUGIN_NAME; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   share_type           dpn_transferred = 0;                       ///< DPN transferred from account to account
   share_type           dbd_transferred = 0;                         ///< DBD transferred from account to account
   share_type           dbd_paid_as_interest = 0;                    ///< DBD paid as interest
   ...
   share_type           dbd_paid_to_authors = 0;                     ///< Ammount of DBD paid to authors
   ...
   share_type           liquidity_rewards_paid = 0;                  ///< Ammount of DPN paid to market makers
   uint32_t             transfers_to_vesting = 0;                    ///< Transfers of DPN into VESTS
   share_type           dpn_vested = 0;                            ///< Ammount of DPN vested
   ...
   share_type           vests_withdrawn = 0;                         ///< Ammount of VESTS withdrawn to DPN
...
} } // dpn::blockchain_statistics

FC_REFLECT( dpn::blockchain_statistics::bucket_object,
   (id)
   (open)
   (seconds)
   (blocks)
...
CHAINBASE_SET_INDEX_TYPE( dpn::blockchain_statistics::bucket_object, dpn::blockchain_statistics::bucket_index )
