...
#include <dpn/chain/operation_notification.hpp>
...
struct operation_process
{
   ...
   void operator()( const transfer_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.amount.symbol == DPN_SYMBOL )
            b.dpn_transferred += op.amount.amount;
         else
            b.dbd_transferred += op.amount.amount;
      });
   }
   ...
   void operator()( const interest_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.dbd_paid_as_interest += op.interest.amount;
      });
   }
   ...
   void operator()( const author_reward_operation& op )const
   {
      ...
      {
         b.payouts++;
         b.dbd_paid_to_authors += op.dbd_payout.amount;
   ...
   void operator()( const fill_vesting_withdraw_operation& op )const
   {
      ...
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.deposited.symbol == DPN_SYMBOL )
   ...
   void operator()( const convert_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.dbd_conversion_requests_created++;
         b.dbd_to_be_converted += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_convert_request_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.dbd_conversion_requests_filled++;
         b.dpn_converted += op.amount_out.amount;
      });
   }
};
...
void blockchain_statistics_plugin_impl::pre_operation( const operation_notification& o )
{
   ...
   for( const auto& bucket_id : _current_buckets )
   {
      ...
      else if( o.op.which() == operation::tag< withdraw_vesting_operation >::value )
      {
         ...
         auto new_vesting_withdrawal_rate = op.vesting_shares.amount / DPN_VESTING_WITHDRAW_INTERVALS;
         if( op.vesting_shares.amount > 0 && new_vesting_withdrawal_rate == 0 )
            new_vesting_withdrawal_rate = 1;

         if( !db.has_hardfork( DPN_HARDFORK_0_1 ) )
...
DPN_DEFINE_PLUGIN( blockchain_statistics, dpn::blockchain_statistics::blockchain_statistics_plugin );
