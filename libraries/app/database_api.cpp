...
//   idump((query));
        ...
        vector<discussion> database_api::get_discussions_by_trending(const discussion_query &query) const {
            ...
            return get_discussions(query, tag, parent, tidx, tidx_itr, [&](const comment_object &c) {
        ...
        vector<discussion> database_api::get_discussions_by_promoted(const discussion_query &query) const {
            ...
            return get_discussions(query, tag, parent, tidx, tidx_itr, [&](const comment_object &c) {
                if (query.select_metadata_tags.size()) {
                    tags::comment_metadata meta = fc::json::from_string(c.json_metadata).template as<tags::comment_metadata>();
        ...
        vector<discussion> database_api::get_discussions_by_feed(const discussion_query &query) const {
            ...
            while (result.size() < query.limit && feed_itr != f_idx.end()) {
                ...
                try {
                    discussion push_discussion = get_discussion(feed_itr->comment);

                    if (query.select_metadata_tags.size()) {
                        tags::comment_metadata meta = fc::json::from_string(push_discussion.json_metadata).as<tags::comment_metadata>();
        ...
        vector<discussion> database_api::get_discussions_by_blog(const discussion_query &query) const {
            ...
            const auto &tag_idx = my->_db.get_index_type<tags::tag_index>().indices().get<tags::by_comment>();
            ...
            while (result.size() < query.limit && blog_itr != b_idx.end()) {
                ...
                try {
                    ...
                    discussion push_discussion = get_discussion(blog_itr->comment);
