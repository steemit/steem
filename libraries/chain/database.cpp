...
// [1] http://stackoverflow.com/questions/8016780/undefined-reference-to-static-constexpr-char
        ...
        void database::open(const fc::path &data_dir, uint64_t initial_supply) {
            try {
                ...
                _block_id_to_block.open(
                        data_dir / "database" / "block_num_to_block");
                ...
                if (last_block.valid()) {
                    ...
                    if (last_block->id() != head_block_id()) {
                        FC_ASSERT(head_block_num() ==
                                  0, "last block ID does not match current chain state",
        ...
        void database::reindex(fc::path data_dir) {
            try {
                ...
                auto reindex_range = [&](uint32_t start_block_num, uint32_t last_block_num, uint32_t skip, bool do_push) {
                    for (uint32_t i = start_block_num;
                         i <= last_block_num; ++i) {
                        if (i % 100000 == 0) {
                            std::cerr << "   "
                                      << double(i * 100) / last_block_num
                                      << "%   " << i << " of " << last_block_num
                                      << "   \n";
                        }
                ...
                const uint32_t initial_undo_blocks =
                        STEEMIT_MAX_TIME_UNTIL_EXPIRATION /
                        STEEMIT_BLOCK_INTERVAL + 5;

                uint32_t first = 1;

                if (last_block_num_in_file > initial_undo_blocks) {
                    uint32_t last =
                            last_block_num_in_file - initial_undo_blocks;
        ...
        bool database::is_known_block(const block_id_type &id) const {
            return _fork_db.is_known_block(id) ||
                   _block_id_to_block.contains(id);
        }
        ...
        void database::update_account_bandwidth(const account_object &a, uint32_t trx_size) {
            ...
            if (props.total_vesting_shares.amount > 0) {
                modify(a, [&](account_object &acnt) {
                    acnt.lifetime_bandwidth +=
                            trx_size * STEEMIT_BANDWIDTH_PRECISION;

                    auto now = head_block_time();
                    auto delta_time = (now -
                                       a.last_bandwidth_update).to_seconds();
                    ...
                    if (delta_time >=
                        ...
                        acnt.average_bandwidth = (old_weight + new_weight) /
                                                 (STEEMIT_BANDWIDTH_AVERAGE_WINDOW_SECONDS);
                    }

                    if (props.total_vesting_shares.amount > 0) {
                        FC_ASSERT(a.vesting_shares.amount >
                                  0, "only accounts with a positive vesting balance may transact");
        ...
        void database::update_account_market_bandwidth(const account_object &a, uint32_t trx_size) {
            ...
            if (props.total_vesting_shares.amount > 0) {
                modify(a, [&](account_object &acnt) {
                    auto now = head_block_time();
                    auto delta_time = (now -
                                       a.last_market_bandwidth_update).to_seconds();
                    uint64_t N = trx_size * STEEMIT_BANDWIDTH_PRECISION;
                    if (delta_time >=
                        STEEMIT_BANDWIDTH_AVERAGE_WINDOW_SECONDS) {
                            acnt.average_market_bandwidth = N;
                    } else {
                        auto old_weight = acnt.average_market_bandwidth *
                                          (STEEMIT_BANDWIDTH_AVERAGE_WINDOW_SECONDS -
                                           delta_time);
                        auto new_weight = delta_time * N;
                        acnt.average_market_bandwidth =
                    ...
                    if (props.total_vesting_shares.amount > 0) {
                        ...
                        fc::uint128 max_virtual_bandwidth(
                                ...
                                10); /// only 10% of bandwidth can be market
 ...
 */
        ...
        string database::get_scheduled_witness(uint32_t slot_num) const {
            ...
            return wso.current_shuffled_witnesses[current_aslot %
                                                  wso.current_shuffled_witnesses.size()];
        }
       ...
       */
        ...
        void database::update_witness_schedule4() {
            ...
            while (mitr != pow_idx.end() &&
                ...
                if (selected_voted.find(mitr->get_id()) ==
                    selected_voted.end()) {
            ...
            for (auto witness_count =
                 ...
                 witness_count < STEEMIT_MAX_MINERS;
                 ++sitr) {
                ...
                if (selected_miners.find(sitr->get_id()) ==
                    selected_miners.end()
                    && selected_voted.find(sitr->get_id()) ==
                       selected_voted.end()) {
            ...
            if (head_block_num() > 14400) {
                FC_ASSERT(active_witnesses.size() ==
                          STEEMIT_MAX_MINERS, "number of active witnesses does not equal STEEMIT_MAX_MINERS",
            ...
            if (has_hardfork(STEEMIT_HARDFORK_0_5__54)) {
                ...
                for (uint32_t i = 0;
                     i < wso.current_shuffled_witnesses.size(); i++) {
            ...
            modify(wso, [&](witness_schedule_object &_wso) {
                ...
                for (uint32_t i = 0;
                     i < _wso.current_shuffled_witnesses.size(); ++i) {
                ...
                _wso.next_shuffle_block_num = head_block_num() +
                                              _wso.current_shuffled_witnesses.size();
 ...
 */
         ...
         for( const string& w : active_witnesses )
            ...
            */
                    ...
                    for (uint32_t i = 0;
                        ...
                        uint32_t jmax =
                                _wso.current_shuffled_witnesses.size() - i;
                        uint32_t j = i + k % jmax;
                        std::swap(_wso.current_shuffled_witnesses[i],
                                _wso.current_shuffled_witnesses[j]);
                    }
        ...
        void database::update_median_witness_props() {
            ...
            active.reserve(wso.current_shuffled_witnesses.size());
            ...
            modify(wso, [&](witness_schedule_object &_wso) {
                _wso.median_props.account_creation_fee = active[active.size() /
            ...
            modify(get_dynamic_global_properties(), [&](dynamic_global_property_object &p) {
                p.maximum_block_size = active[active.size() /
            ...
            modify(get_dynamic_global_properties(), [&](dynamic_global_property_object &p) {
                p.sbd_interest_rate = active[active.size() /
                ...
                if (comment.net_rshares > 0) {
                    ...
                    if (reward_tokens > 0) {
                        ...
                        if (sbd_created.symbol == SBD_SYMBOL) {
                            adjust_total_payout(comment, sbd_created +
                                                         to_sbd(asset(vesting_steem, STEEM_SYMBOL)), to_sbd(asset(
                                    reward_tokens.to_uint64() -
                                    author_tokens, STEEM_SYMBOL)));
                        } else {
                            adjust_total_payout(comment, to_sbd(asset(
                                    vesting_steem +
                                    sbd_steem, STEEM_SYMBOL)), to_sbd(asset(
                                    reward_tokens.to_uint64() -
                                    author_tokens, STEEM_SYMBOL)));
                        }
 ...
 */
        ...
        asset database::get_pow_reward() const {
            ...
            static_assert(STEEMIT_BLOCK_INTERVAL ==
                          3, "this code assumes a 3-second time interval");
            static_assert(
                    STEEMIT_MAX_MINERS == 21, "this code assumes 21 per round");
...
//////////////////// private methods ////////////////////
   ...
   FC_CAPTURE_AND_RETHROW( (next_block) );*/
        ...
        void database::_apply_block(const signed_block &next_block) {
            try {
                ...
                FC_ASSERT((skip & skip_merkle_check) ||
                          ...
                          next_block.calculate_merkle_root(), "Merkle check failed", ("next_block.transaction_merkle_root", next_block.transaction_merkle_root)("calc", next_block.calculate_merkle_root())("next_block", next_block)("id", next_block.id()));
                ...
                {
                    FC_ASSERT(get_witness(next_block.witness).running_version >=
                              hardfork_property_id_type()(*this).current_hardfork_version,
       ...
       */
        ...
        void database::update_median_feed() {
            try {
                ...
                vector<price> feeds;
                feeds.reserve(wso.current_shuffled_witnesses.size());
            ...
            modify(_dgp, [&](dynamic_global_property_object &dgp) {
                ...
                for (uint32_t i = 0; i < missed_blocks + 1; i++) {
                    ...
                    dgp.recent_slots_filled =
                            (dgp.recent_slots_filled << 1) + (i == 0 ? 1 : 0);
       ...
       */
                ...
                dgp.max_virtual_bandwidth =
                        (dgp.maximum_block_size * dgp.current_reserve_ratio *
            ...
            _undo_db.set_max_size(
                    _dgp.head_block_number - _dgp.last_irreversible_block_num +
                    1);
        ...
        void database::update_signing_witness(const witness_object &signing_witness, const signed_block &new_block) {
            ...
            uint64_t new_block_aslot =
                    dpo.current_aslot + get_slot_at_time(new_block.timestamp);
    ...
    */
                ...
                modify(dpo, [&](dynamic_global_property_object &_dpo) {
                    if (head_block_num() > STEEMIT_MAX_MINERS) {
                        _dpo.last_irreversible_block_num =
                                head_block_num() - STEEMIT_MAX_MINERS;
                    }
                });
                return;
            }
            ...
            size_t offset = (
                    (STEEMIT_100_PERCENT - STEEMIT_IRREVERSIBLE_THRESHOLD) *
        ...
        bool database::has_hardfork(uint32_t hardfork) const {
            return hardfork_property_id_type()(*this).processed_hardforks.size() >
                   hardfork;
        }
        ...
        void database::set_hardfork(uint32_t hardfork, bool apply_now) {
            ...
            for (int i = hardforks.last_hardfork + 1;
...
// Low memory nodes only need immediate child count, full nodes track total children
        ...
        const category_object &database::get_category(const string &name) const {
            ...
            FC_ASSERT(cat !=
                      nullptr, "Unable to find category ${c}", ("c", name));
            return *cat;
