        ...
        void account_update_evaluator::do_apply(const account_update_operation &o) {
            ...
            if (o.owner) {
                ...
                if (db().has_hardfork(STEEMIT_HARDFORK_0_11))
                    FC_ASSERT(
                            db().head_block_time() - account.last_owner_update >
                            STEEMIT_OWNER_UPDATE_LIMIT, "can only update owner authority once a minute");
#endif
        ...
        void delete_comment_evaluator::do_apply(const delete_comment_operation &o) {
            if (db().has_hardfork(STEEMIT_HARDFORK_0_10)) {
                ...
                FC_ASSERT(!(auth.owner_challenged ||
                            auth.active_challenged), "cannot process operation because account is currently challenged");
            }
            ...
            if (db().is_producing()) {
                FC_ASSERT(comment.net_rshares <=
                          0, "comment cannot have any net positive votes");
            }
        ...
        void comment_options_evaluator::do_apply(const comment_options_operation &o) {
            ...
            if (!o.allow_curation_rewards || !o.allow_votes ||
                ...
                FC_ASSERT(comment.abs_rshares ==
                          0, "operations contains options that are only updatedable on an account with no rshares");

            FC_ASSERT(o.extensions.size() ==
                      0, "extensions are not currently supported");
            FC_ASSERT(comment.allow_curation_rewards >=
                      o.allow_curation_rewards, "cannot re-enable curation rewards");
            FC_ASSERT(comment.allow_votes >=
                      o.allow_votes, "cannot re-enable votes");
            FC_ASSERT(comment.max_accepted_payout >=
                      o.max_accepted_payout, "cannot accept a greater payout");
            FC_ASSERT(comment.percent_steem_dollars >=
                      o.percent_steem_dollars, "cannot accept a greater percent Steem Dollars");
            ...
            try {
                if (db().is_producing() ||
                    ...
                    FC_ASSERT(o.title.size() + o.body.size() +
                              o.json_metadata.size(), "something should change");
                ...
                if (itr == by_permlink_idx.end()) {
                    ...
                    if (db().has_hardfork(STEEMIT_HARDFORK_0_12__176)) {
                        if (o.parent_author.size() == 0)
                            FC_ASSERT((now - auth.last_root_post) >
                                      STEEMIT_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes", ("now", now)("auth.last_root_post", auth.last_root_post));
                        else
                    ...
                    if (db().has_hardfork(STEEMIT_HARDFORK_0_12__176) &&
                        ...
                        uint32_t old_weight = uint32_t((post_bandwidth *
                        ...
                        reward_weight = uint16_t(std::min(
                                (STEEMIT_POST_WEIGHT_CONSTANT *
                                 STEEMIT_100_PERCENT) / (post_bandwidth *
                                                         post_bandwidth), uint64_t(STEEMIT_100_PERCENT)));
                    }
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
                ...
                } else // start edit case
                {
                    ...
                    db().modify(comment, [&](comment_object &com) {
                        ...
                        if (!parent) {
                            ...
                            FC_ASSERT(com.parent_permlink ==
                                      o.parent_permlink, "The permlink of a comment cannot change");
                        } else {
                            FC_ASSERT(com.parent_author ==
        ...
        void escrow_transfer_evaluator::do_apply(const escrow_transfer_operation &o) {
            try {
                ...
                FC_ASSERT(o.ratification_deadline >
                          db().head_block_time(), "ratification deadline must be after head block time");
                FC_ASSERT(o.escrow_expiration >
                          db().head_block_time(), "escrow expiration must be after head block time");
                ...
                FC_ASSERT(from_account.balance >=
                          steem_spent, "account cannot cover steem costs of escrow");
                FC_ASSERT(from_account.sbd_balance >=
                          sbd_spent, "account cannot cover sbd costs of escrow");
        ...
        void escrow_approve_evaluator::do_apply(const escrow_approve_operation &o) {
            try {
                ...
                FC_ASSERT(escrow.to ==
                          o.to, "op 'to' does not match escrow 'to'");
                FC_ASSERT(escrow.agent ==
                          o.agent, "op 'agent' does not match escrow 'agent'");
                FC_ASSERT(escrow.ratification_deadline >=
                          db().head_block_time(), "escrow ratification deadline is before head block time");
        ...
        void escrow_dispute_evaluator::do_apply(const escrow_dispute_operation &o) {
            try {
                ...
                FC_ASSERT(db().head_block_time() <
                          e.escrow_expiration, "disputes must be raised before expiration");
                FC_ASSERT(e.to_approved &&
                          e.agent_approved, "escrow must be approved by all parties before a dispute can be raised");
        ...
        void escrow_release_evaluator::do_apply(const escrow_release_operation &o) {
            try {
                ...
                FC_ASSERT(e.steem_balance >=
                          o.steem_amount, "Release amount exceeds escrow balance");
                FC_ASSERT(e.sbd_balance >=
                          o.sbd_amount, "Release amount exceeds escrow balance");
                FC_ASSERT(e.to == o.to, "op 'to' does not match escrow 'to'");
                FC_ASSERT(e.agent ==
                          o.agent, "op 'agent' does not match escrow 'agent'");
                FC_ASSERT(o.receiver == e.from || o.receiver ==
                                                  e.to, "Funds must be released to 'from' or 'to'");
                FC_ASSERT(e.to_approved &&
                ...
                if (e.disputed) {
                    FC_ASSERT(o.who ==
                              e.agent, "'agent' must release funds for a disputed escrow");
                } else {
                    FC_ASSERT(o.who == e.from || o.who ==
                                                 e.to, "Only 'from' and 'to' can release from a non-disputed escrow");

                    if (e.escrow_expiration > db().head_block_time()) {
                        // If there is no dispute and escrow has not expired, either party can release funds to the other.
                        if (o.who == e.from) {
                            FC_ASSERT(o.receiver ==
                                      e.to, "'from' must release funds to 'to'");
                        } else if (o.who == e.to) {
                            FC_ASSERT(o.receiver ==
                                      e.from, "'to' must release funds to 'from'");
                        }
                    }
                }
        ...
        void transfer_to_vesting_evaluator::do_apply(const transfer_to_vesting_operation &o) {
            ...
            FC_ASSERT(db().get_balance(from_account, STEEM_SYMBOL) >=
                      o.amount, "account does not have sufficient STEEM for the transfer");
            db().adjust_balance(from_account, -o.amount);
        ...
        void withdraw_vesting_evaluator::do_apply(const withdraw_vesting_operation &o) {
            ...
            FC_ASSERT(account.vesting_shares >=
                      asset(0, VESTS_SYMBOL), "account does not have sufficient Steem Power for withdraw");
            FC_ASSERT(account.vesting_shares >=
            ...
            if (o.vesting_shares.amount == 0) {
                ...
                db().modify(account, [&](account_object &a) {
                    auto new_vesting_withdraw_rate = asset(
                            ...
                            STEEMIT_VESTING_WITHDRAW_INTERVALS, VESTS_SYMBOL);
        ...
        void set_withdraw_vesting_route_evaluator::do_apply(const set_withdraw_vesting_route_operation &o) {
            try {
                ...
                FC_ASSERT(total_percent <=
                          STEEMIT_100_PERCENT, "More than 100% of vesting allocated to destinations");
            }
        ...
        void account_witness_proxy_evaluator::do_apply(const account_witness_proxy_operation &o) {
            ...
            if (o.proxy.size()) {
                ...
                flat_set<account_id_type> proxy_chain({account.get_id(),
                                                       new_proxy.get_id()
                });
            ...
            try {
                ...
                if (db().has_hardfork(STEEMIT_HARDFORK_0_10))
                    FC_ASSERT(!(voter.owner_challenged ||
                                voter.active_challenged), "Account is currently challenged");
         ...
         db().modify( *itr, [&]( comment_vote_object& cvo )
         {
            ...
            cvo.last_update = db().head_block_time();
                ...
                FC_ASSERT(used_power <=
                          current_power, "Account does not have enough power for vote");
      ...
      **/
                    ...
                    FC_ASSERT(itr->num_changes <
                              STEEMIT_MAX_VOTE_CHANGES, "Cannot change vote again");

                    if (db().is_producing() ||
                        db().has_hardfork(STEEMIT_HARDFORK_0_6__112))
                        FC_ASSERT(itr->vote_percent !=
                                  o.weight, "Changing your vote requires actually changing you vote.");
                    ...
                    if (itr->rshares < rshares &&
                        ...
                        FC_ASSERT(db().head_block_time() <
                                  db().calculate_discussion_payout_time(comment) -
                                  STEEMIT_UPVOTE_LOCKOUT, "Cannot increase payout withing last minute before payout");
        ...
        void pow_apply(database &db, Operation o) {
            ...
            FC_ASSERT(worker_account.active.num_auths() ==
                      1, "miners can only have one key auth");
            FC_ASSERT(worker_account.active.key_auths.size() ==
                      1, "miners may only have one key auth");
            FC_ASSERT(worker_account.active.key_auths.begin()->first ==
        ...
        void pow2_evaluator::do_apply(const pow2_operation &o) {
            ...
            FC_ASSERT(work.pow_summary <
                      target_pow, "insufficient work difficulty");

            FC_ASSERT(o.props.maximum_block_size >=
                      STEEMIT_MIN_BLOCK_SIZE_LIMIT *
                      2, "maximum block size is too small");
        ...
        void convert_evaluator::do_apply(const convert_operation &o) {
            ...
            FC_ASSERT(db().get_balance(owner, o.amount.symbol) >=
                      o.amount, "account does not sufficient balance");
            ...
            db().create<convert_request_object>([&](convert_request_object &obj) {
                ...
                obj.amount = o.amount;
                obj.conversion_date = db().head_block_time() +
                                      STEEMIT_CONVERSION_DELAY; // 1 week
            });
      ...
      db().modify( challenged, [&]( account_object& a )
      {
        ...
        void request_account_recovery_evaluator::do_apply(const request_account_recovery_operation &o) {
            ...
            if (account_to_recover.recovery_account.length())   // Make sure recovery matches expected recovery account
                ...
                FC_ASSERT(
                        db().get_index_type<witness_index>().indices().get<by_vote_name>().begin()->owner ==
        ...
        void transfer_to_savings_evaluator::do_apply(const transfer_to_savings_operation &op) {
            ...
            FC_ASSERT(db().get_balance(from, op.amount.symbol) >=
                      op.amount, "Account does not have sufficient funds for transfer.");
        ...
        void reset_account_evaluator::do_apply(const reset_account_operation &op) {
            ...
            FC_ASSERT((db().head_block_time() - acnt.last_bandwidth_update) >
                      fc::days(60));
