    ...
    namespace chain {
        ...
        void block_database::open(const fc::path &dbdir) {
            try {
                ...
                _block_num_to_pos.exceptions(
                        std::ios_base::failbit | std::ios_base::badbit);
                _blocks.exceptions(
                        std::ios_base::failbit | std::ios_base::badbit);

                if (!fc::exists(dbdir / "index")) {
                    _block_num_to_pos.open((dbdir /
                                            "index").generic_string().c_str(),
        ...
        void block_database::remove(const block_id_type &id) {
            try {
                ...
                if (e.block_id == id) {
                    e.block_size = 0;
                    _block_num_to_pos.seekp(
                            sizeof(e) * block_header::num_from_id(id));
                    _block_num_to_pos.write((char *)&e, sizeof(e));
                }
        ...
        block_id_type block_database::fetch_block_id(uint32_t block_num) const {
            ...
            FC_ASSERT(e.block_id !=
                      block_id_type(), "Empty block_id in block_database (maybe corrupt on disk?)");
            return e.block_id;
        }
