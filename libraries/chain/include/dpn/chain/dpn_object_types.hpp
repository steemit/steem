...
#include <dpn/chain/multi_index_types.hpp>

#ifndef ENABLE_MIRA
#define DPN_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
      object_type () = delete;                           \
   public:
#else
#define DPN_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
...
using dpn::protocol::chain_id_type;
...
template<> struct is_static_length< dpn::protocol::account_name_type > : public boost::true_type {};
template<> struct is_static_length< dpn::protocol::asset_symbol_type > : public boost::true_type {};
template<> struct is_static_length< dpn::protocol::asset > : public boost::true_type {};
template<> struct is_static_length< dpn::protocol::price > : public boost::true_type {};
...
#ifndef ENABLE_MIRA
inline void to_variant( const dpn::chain::shared_string& s, variant& var )
{
   var = fc::string( dpn::chain::to_string( s ) );
}

inline void from_variant( const variant& var, dpn::chain::shared_string& s )
{
...
{
   std::string str = dpn::chain::to_string( ss );
   fc::raw::pack( s, str );
}
...
#ifndef ENABLE_MIRA
template< typename T >
T unpack_from_vector( const dpn::chain::buffer_type& s )
{
   try
   {
      T tmp;
...
FC_REFLECT_ENUM( dpn::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_metadata_object_type)
                 (account_authority_object_type)
...
#ifndef ENABLE_MIRA
FC_REFLECT_TYPENAME( dpn::chain::shared_string )
#endif

FC_REFLECT_ENUM( dpn::chain::bandwidth_type, (post)(forum)(market) )
