...
namespace dpn { namespace chain {
   ...
   {
      DPN_STD_ALLOCATOR_CONSTRUCTOR( transaction_object )
...
FC_REFLECT( dpn::chain::transaction_object, (id)(packed_trx)(trx_id)(expiration) )
CHAINBASE_SET_INDEX_TYPE( dpn::chain::transaction_object, dpn::chain::transaction_index )

namespace helpers
{
   template <>
   class index_statistic_provider<dpn::chain::transaction_index>
   {
   public:
      typedef dpn::chain::transaction_index IndexType;
      typedef typename dpn::chain::transaction_object::t_packed_trx t_packed_trx;
