...
void smt_cap_reveal_evaluator::do_apply( const smt_cap_reveal_operation& o )
{
   ...
   FC_ASSERT( token_ico->dpn_units_min_cap < 0 || token_ico->dpn_units_hard_cap < 0, "Both min cap and max hard cap have already been revealed" );

   if( token_ico->dpn_units_min_cap < 0 )
      try
      {
         o.cap.validate( token_ico->capped_generation_policy.min_dpn_units_commitment );
         _db.modify( *token_ico, [&]( smt_ico_object& ico_object )
         {
            ico_object.dpn_units_min_cap = o.cap.amount;
         });
         return;
      }
      catch( const fc::exception& e )
      {
         if( token_ico->dpn_units_hard_cap >= 0 )
            throw;
      }

   o.cap.validate( token_ico->capped_generation_policy.hard_cap_dpn_units_commitment );
   _db.modify( *token_ico, [&]( smt_ico_object& ico_object )
   {
      ico_object.dpn_units_hard_cap = o.cap.amount;
   });
}
