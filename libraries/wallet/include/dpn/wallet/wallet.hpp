...
{
   public:
      wallet_api( const wallet_data& initial_data, const dpn::protocol::chain_id_type& _dpn_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of DPN and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param dpn_fee The amount of the fee to be paid with DPN
      ...
      /**
       * Transfer funds from one account to another. DPN and DBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 DPN"
      ...
      /**
       * Transfer funds from one account to another using escrow. DPN and DBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param agent The account acting as the agent in case of dispute
       * @param escrow_id A unique id for the escrow transfer. (from, escrow_id) must be a unique pair
       * @param dbd_amount The amount of DBD to transfer
       * @param dpn_amount The amount of DPN to transfer
       ...
       * @param escrow_id A unique id for the escrow transfer
       * @param dbd_amount The amount of DBD that will be released
       * @param dpn_amount The amount of DPN that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer DPN into a vesting fund represented by vesting shares (VESTS). VESTS are required to vesting
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * VESTS are protected against dilution up until 90% of DPN is vesting.
       *
       * @param from The account the DPN is coming from
       * @param to The account getting the VESTS
       * @param amount The amount of DPN to vest i.e. "100.00 DPN"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *    withdrawn and deposited back as DPN. i.e. "10.000000 VESTS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the VESTS are withdrawn from.
       * @param to   The account receiving either VESTS or DPN.
      ...
      /**
       *  This method will convert DBD to DPN at the current_median_history price one
       *  week from the time it is executed. This method depends upon there being a valid price feed.
       *
       *  @param from The account requesting conversion of its DBD i.e. "1.000 DBD"
       *  @param amount The amount of DBD to convert
       *  @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction convert_dbd(
         string from,
         condenser_api::legacy_asset amount,
         bool broadcast = false );

      /**
       * A witness can public a price feed for the DPN:DBD market. The median price feed is used
       * to process conversion requests from DBD to DPN.
       *
       ...
       *                       operations defined in `dpn/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for DPN:DBD
       *
       ...
       *  @param amount_to_sell The amount of either DBD or DPN you wish to sell
      ...
      /**
       * Vote on a comment to be paid DPN
       *
       ...
       * @param daily_pay  - the amount of DBD that is being requested to be paid out daily,
...
FC_REFLECT( dpn::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( dpn::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( dpn::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( dpn::wallet::authority_type, (owner)(active)(posting) )

FC_API( dpn::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( dpn::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
