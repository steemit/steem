...
struct smt_capped_generation_policy
{
   ...
   smt_cap_commitment  min_dpn_units_commitment;
   smt_cap_commitment  hard_cap_dpn_units_commitment;
...
FC_REFLECT(
   dpn::protocol::smt_base_operation,
   (control_account)
   (symbol)
)

FC_REFLECT(
   dpn::protocol::smt_executor_base_operation,
   (executor)
   (symbol)
)
...
FC_REFLECT(
   dpn::protocol::smt_cap_commitment,
   (lower_bound)
   (upper_bound)
   (hash)
   )
...
FC_REFLECT(
   dpn::protocol::smt_revealed_cap,
   (amount)
   (nonce)
   )

FC_REFLECT_DERIVED(
   dpn::protocol::smt_cap_reveal_operation,
   (dpn::protocol::smt_base_operation),
   (cap)
   (extensions)
   )
...
FC_REFLECT(
   ...
   (pre_soft_cap_unit)
   (post_soft_cap_unit)
   (min_dpn_units_commitment)
   (hard_cap_dpn_units_commitment)
   (soft_cap_percent)
   (min_unit_ratio)
   (max_unit_ratio)
   (extensions)
...
FC_REFLECT_DERIVED(
   dpn::protocol::smt_refund_operation,
   (dpn::protocol::smt_executor_base_operation),
   (contributor)
   (contribution_id)
   (amount)
