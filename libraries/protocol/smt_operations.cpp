...
void smt_setup_operation::validate()const
{
   ...
   FC_ASSERT( steem_units_min_cap <= steem_units_hard_cap, "Steem units min cap must less than or equal to steem units hard cap" );
   FC_ASSERT( steem_units_min_cap >= SMT_MIN_SOFT_CAP_STEEM_UNITS, "Steem units min cap must be greater than or equal to ${n}", ("n", SMT_MIN_SOFT_CAP_STEEM_UNITS) );
