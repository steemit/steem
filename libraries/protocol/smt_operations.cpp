...
void smt_capped_generation_policy::validate()const
{
   ...
   min_dpn_units_commitment.validate();
   hard_cap_dpn_units_commitment.validate();
   ...
   FC_ASSERT( hard_cap_dpn_units_commitment.lower_bound >= SMT_MIN_HARD_CAP_DPN_UNITS );
   ...
   static_assert( uint64_t( DPN_MAX_SHARE_SUPPLY ) < (std::numeric_limits< uint64_t >::max() / DPN_100_PERCENT), "Overflow check failed" );
   uint64_t min_soft_cap = (uint64_t( hard_cap_dpn_units_commitment.lower_bound.value ) * soft_cap_percent) / DPN_100_PERCENT;
   uint64_t max_soft_cap = (uint64_t( hard_cap_dpn_units_commitment.upper_bound.value ) * soft_cap_percent) / DPN_100_PERCENT;

   // we want soft cap to be large enough we don't see quantization effects
   FC_ASSERT( min_soft_cap >= SMT_MIN_SOFT_CAP_DPN_UNITS );

   // We want to prevent the following from overflowing DPN_MAX_SHARE_SUPPLY:
   // max_tokens_created = (u1.tt * sc + u2.tt * (hc-sc)) * min_unit_ratio
   // max_dpn_accepted =  u1.st * sc + u2.st * (hc-sc)

   // hc / max_unit_ratio is the saturation point

   uint128_t sc = max_soft_cap;
   uint128_t hc_sc = hard_cap_dpn_units_commitment.upper_bound.value - max_soft_cap;

   uint128_t max_tokens_created = (pre_soft_cap_unit.token_unit_sum() * sc + post_soft_cap_unit.token_unit_sum() * hc_sc) * min_unit_ratio;
   uint128_t max_share_supply_u128 = uint128_t( DPN_MAX_SHARE_SUPPLY );

   FC_ASSERT( max_tokens_created <= max_share_supply_u128 );

   uint128_t max_dpn_accepted = (pre_soft_cap_unit.dpn_unit_sum() * sc + post_soft_cap_unit.dpn_unit_sum() * hc_sc);
