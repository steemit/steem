...
namespace steemit {
    namespace chain2 {
        ...
        public:
            ...
            fc::signal<void( const operation
            & )>
            pre_apply_operation;
            fc::signal<void( const operation
            & )>
            ...
            struct restore_pending {
                restore_pending(database &db, vector<signed_transaction> &&p)
                        : _db(db), _pending(std::move(p)) {
                }

                restore_pending(restore_pending &&mv)
                        : _db(mv._db), _pending(std::move(mv._pending)) {
                }
